...
\begin{figure}[p]
  ...
  \caption{Where the 363 measurements actually are. \textbf{Top:} all three
  areas in one frame, on the street network each was measured in, coloured by
  the level measured there. \textbf{Bottom:} each area cut to the same
  1.8$\times$1.8\,km box, so the three densities can be compared by eye ---
  panels scaled to their own data would make the sparsest site look the busiest.
  The blank space in the top panel is the argument of this report in one
  picture: the campaign is three islands roughly 11\,km apart, and the model of
  §\ref{sec:ceiling} is asked to generalise across everything between them,
  which was never measured. There is no basemap and no tile server here ---
  roads are the OSMnx extracts the GAMA simulation already runs on
  (\pth{simulation/gama/inputs/}), so nothing on this figure comes from outside
  the repository. \pth{scripts/12\_presentation\_figures.py}.}
  \label{fig:sites}
\end{figure}
...
\noindent Three things in Figure~\ref{fig:sites} bear on everything that
...
\pth{dist\_to\_road} --- and a model fitted on it inherits that bias. And the
areas are not equal: Ocean Park carries half the campaign.
...
\begin{caveat}{Drawing the frame found a problem in it}
Placing the three measured areas on the ward boundaries was meant to be
illustration, and returned a finding instead. \textbf{Ocean Park is not in a
\emph{phuong}.} All 184 of its measurements --- half this campaign --- fall
inside \emph{Xa Gia Lam}, a rural commune, which the frame as specified above
excludes. Hoan Kiem and Vinh Tuy are inside it; Ocean Park is not.

Nothing in §4 changes: the site is real and the measurements are valid. What
changes is the plan. Either the frame widens to admit the peri-urban
\emph{xa} --- in which case ``51 urban \emph{phuong}'' is the wrong description
of it, and the stratum for new peri-urban development has no obvious home ---
or the frame stands and the one area this project knows best contributes
nothing to the design of the next campaign. That is a decision, and it belongs
to whoever fixes the protocol, not to this report.
\end{caveat}
...
(lakes and parks); and construction and industrial sites. The draw is made
across the \textbf{51 urban \emph{phuong}} (wards) of Hanoi under the
administrative division in force since July~2025.
...
\pth{data/processed/hanoi\_wards.geojson} & \pth{scripts/fetch\_hanoi\_wards.py},
  one Overpass query against OpenStreetMap, fetched 28 August 2026 and
  committed. 126 units: 51 \emph{phuong}, 75 \emph{xa}. It is the only script
  in the project that touches the network, which is why it is unnumbered and
