...
\begin{figure}[htbp]
  ...
  \caption{The five screens of the field application. These are vector
    ...
    (\pth{mobile/app/src/main/java/org/noisehanoi/mobile/ui/}), screen by screen
    --- \pth{HomeScreen.kt}, \pth{FormScreen.kt}, \pth{MapScreen.kt},
...
\begin{figure}[htbp]
  ...
  \caption{Level against the source class the operator selected on the form ---
  the one variable in the dataset that is a human judgement rather than a sensor
  ...
  with no reordering needed. It also shows how unbalanced the campaign is ---
  \emph{transportation noise} and \emph{silence} carry three quarters of it.
...
for, and the plan is what makes the negative results of §4 worth having ---
each of them is a constraint the design below has to respect.
...
\begin{itemize}
  ...
  \item \textbf{The consent screen must state what the application uploads} ---
        the audio clip, the coordinates, the timestamp --- and what it does not.
\end{itemize}
...
Figure~\ref{fig:stills}, the three stills & \pth{scripts/make\_video\_stills.py}
  --- redaction by \pth{yolov8n}, then checked by eye. No raw frame is written
  to disk. \\
