...
\midrule
1--12 June & Repository opened; Sunbird pipeline reproduced;
 large surrogate and convention-invariant v2 features trained; Barcelona diagnostic; field forms v2 and the
 construction register written; GAMA plan
 drafted. \\
10 June -- 22 July & \textbf{Field campaign.} Carried mainly by the two interns, with
 ...
 Collect against a KoboToolbox server, three contrasted typologies. Pipeline re-run as the sample grew, from 107 points to 220. Transfer
 abandoned in favour of direct local training on 30 June. \\
15--31 July & Repository restructured, YOLO vehicle counting, weather analysis,
 seven-page report; first deck and GAMA model.
...
6 August & Measured flow restricted to a 150\,m corridor around measurement
 points. \\
11--12 August & \textbf{Restructuring for handover:} target layout, installable \pth{noise\_hanoi} package,
 Makefile and first tests, pickled Uganda models replaced by
 text boosters, licences and \pth{CITATION.cff}, handover, methodology,
 full French$\to$English pass across scripts, notebooks, GAMA and docs. \\
20--21 August & \textbf{Android field application} ( and the
 \pth{feat/mobile-app} branch); Uganda transfer made reproducible; presentation brief; report typeset in
 LaTeX; presentation figures, decks and speaker scripts
 ( \ldots). \\
\bottomrule
\end{tabular}
\end{center}
...
contrasted urban typologies, together with 147 timestamped traffic
...
\begin{enumerate}
 ...
 21~August. Actual state on 24 August 2026:
...
\subsection{What this internship leaves behind}

The numbers in this report are bounded by the instrument and by the size of the
sample, and we hold them accordingly: the biases documented in §5.1 are real and
we had no means of removing them. The chain that produces them is a different
matter. Given a reference instrument to anchor the absolute scale and more
measurement points, it would yield better results without a change of design, and
that is what §6 and §7.2 are for. Two things stand on their own without any of that: a published dataset of 363
measurements and 147 video counts under an open licence, which the next
campaign extends instead of starting over, and a working offline application
that collects to the same schema. What was built here is the part that can be
reused; what was measured is the part that has to grow.
