...
% levelColour() in ui/Theme.kt, band for band. The ramp is ordered by
% lightness: the old 55-65 band was an olive (#9E9D24) that sat at the same
% lightness as the 65-70 orange beside it, so the two middle bands -- most of
% the map -- read as one. Each band is now clearly darker than the one under it.
\definecolor{lvlA}{HTML}{14654A}      % < 55
\definecolor{lvlB}{HTML}{F5D06B}      % 55-65
\definecolor{lvlC}{HTML}{EE9422}      % 65-70
\definecolor{lvlD}{HTML}{D6541E}      % 70-80
\definecolor{lvlE}{HTML}{5E0F14}      % > 80
% Darker siblings, for the two places a band colour is used as TEXT rather than
% as a fill: the big dB reading and the map's "you are here" label.
\definecolor{lvlCtext}{HTML}{9A5A05}
\definecolor{lvlDtext}{HTML}{A83A12}
...
\phoneframe{%
  ...
  \appcard[cardfill]{-14}{46}{%
    ...
    \node[font=\fontsize{11}{11}\selectfont, text=lvlDtext, anchor=north west]
      at (0,-14) {71.4 dB};
...
\phoneframe{%
  ...
  \appcard{-8}{18}{%
    ...
    \node[font=\fontsize{9}{9}\selectfont, text=lvlCtext, anchor=north west] at (0,-3.4)
      {68 dB};
