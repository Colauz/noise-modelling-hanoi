...
% \forwhat{EN}{FR} --- the one line that says what the formula is FOR in this
% project: what it predicts, what it decides, what it draws. It sits directly
% under the equation, before the symbols, because "why am I looking at this" is
% the question a reader has first and the one an explanation usually answers
% last.
\newcommand{\forwhat}[2]{%
  \par\vspace{-1mm}\noindent
  {\footnotesize\color{good}\textsc{what it is for \; $\cdot$ \; à quoi elle sert}}
  \par\vspace{1mm}\noindent
...
\forwhat{Turning what the microphone hears into a number. \must{In this project it
    is the reason we can compare places and hours, but never state an absolute
    level} --- because we never fixed our reference.}{Convertir ce qu'entend le microphone en un nombre. \must{Dans ce projet,
    c'est la raison pour laquelle nous pouvons comparer des lieux et des heures,
    mais jamais énoncer un niveau absolu} --- faute d'avoir fixé notre référence.}
...
\forwhat{\must{Defining what one field measurement \emph{is}.} Each of the 363
    points is one evaluation of this integral. It is what the app computes over
    25 seconds, and the quantity every model in the deck tries to predict.}{\must{Définir ce qu'\emph{est} une mesure de terrain.} Chacun des 363
    points est une évaluation de cette intégrale. C'est ce que l'application
    calcule sur 25 secondes, et la grandeur que tous les modèles tentent de
    prédire.}
...
\forwhat{The filter applied to the microphone signal \emph{before} anything else.
    \must{It is what makes our number comparable to every published noise
    figure} --- and the app's implementation is checked against the IEC table by a
    test.}{Le filtre appliqué au signal du microphone \emph{avant} tout le reste.
    \must{C'est ce qui rend notre nombre comparable à toute mesure de bruit
    publiée} --- et l'implémentation de l'application est vérifiée contre la table
    IEC par un test.}
...
\forwhat{\must{Every time two noises are combined or averaged.} The two road terms
    adding inside the kernel, the mean level of a site, the traffic scenario ---
    all three go through this line, and skipping it produced the $-7.0\dB$ error
    twice.}{\must{Chaque fois que deux bruits sont combinés ou moyennés.} Les deux
    termes de route qui s'additionnent dans le noyau, le niveau moyen d'un site,
    le scénario de trafic --- les trois passent par cette ligne, et l'oublier a
    produit deux fois l'erreur de $-7,0\dB$.}
...
\forwhat{\must{Predicting a level where nobody measured.} This is the model that
    draws all 5\,587 cells of the map, the one embedded in the phone, and the one
    the whole evaluation section is about.}{\must{Prédire un niveau là où personne n'a mesuré.} C'est le modèle qui
    dessine les 5\,587 cellules de la carte, celui embarqué dans le téléphone, et
    celui dont parle toute la section évaluation.}
...
\eq{\text{point source: } E \propto \frac{1}{d^{2}} \;\Rightarrow\; -6.02\dB
    ...
    \text{ per doubling}}
\forwhat{\must{Choosing the shape of the kernel before fitting anything.} It is
    the reason a two-parameter distance law competes at all --- and the reason
    distance to the road carries nearly all the spatial signal.}{\must{Choisir la forme du noyau avant tout ajustement.} C'est pourquoi une
    loi de distance à deux paramètres tient la comparaison --- et pourquoi la
    distance à la route porte presque tout le signal spatial.}
...
\forwhat{\must{The traffic slider.} What the map becomes with a fifth of the
    traffic, or three times as much --- in the app, and in the GAMA model it
    drives. It is what turns a static map into a scenario.}{\must{Le curseur de trafic.} Ce que devient la carte avec un cinquième du
    trafic, ou trois fois plus --- dans l'application, et dans le modèle GAMA
    qu'elle pilote. C'est ce qui transforme une carte figée en scénario.}
...
\forwhat{\must{Deciding which model ships.} \texttt{04\_evaluate\_models.py} picks
    the winner on $R^2$ under the reference protocol --- by code, not by hand. MAE
    is the one you say out loud: \must{5\,dB}.}{\must{Décider quel modèle est livré.} \texttt{04\_evaluate\_models.py}
    choisit le gagnant au $R^2$ sous le protocole de référence --- par le code, pas
    à la main. La MAE est celle qu'on dit à voix haute~: \must{5\,dB}.}
...
\forwhat{\must{Making the score honest rather than flattering.} It is the entire
    difference between $R^2 = 0.45$ (withdrawn) and $R^2 = 0.246$ (delivered).
    Same data, same model --- a different split.}{\must{Rendre le score honnête plutôt que flatteur.} C'est toute la
    différence entre $R^2 = 0,45$ (retiré) et $R^2 = 0,246$ (livré). Mêmes
    données, même modèle --- un découpage différent.}
...
\forwhat{\must{The error bars on every score in the deck.} It is what shows the
    top two models are not actually separated --- and a by-point bootstrap would
    have hidden that.}{\must{Les barres d'erreur de chaque score du deck.} C'est ce qui montre
    que les deux premiers modèles ne sont pas réellement séparés --- un bootstrap
    par point l'aurait masqué.}
...
\forwhat{\must{Answering ``isn't 0.25 very low?'' before it is asked.} It turns a
    number that looks weak into a number that is about half of what the data
    physically allows.}{\must{Répondre à «~0,25, ce n'est pas très bas~?~» avant qu'on le
    demande.} Il transforme un chiffre qui semble faible en un chiffre qui vaut
    environ la moitié de ce que les données permettent physiquement.}
