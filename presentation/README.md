...
| Conclusion | 2 | |
| Backup | 3 | **The forest plot of every interval**, the bias interval, what is published |
...
`logos/` currently holds **typographic placeholders**, not the institutions'
marks — see [`logos/README.md`](logos/README.md). Drop the real `cosmos.pdf`,
`vinuni.pdf`, `isima.pdf` and `uca.pdf` in and the deck picks them up with no
edit. Both the title slide and the closing slide draw the band with one
`\logoband` command, so the two cannot drift apart.
